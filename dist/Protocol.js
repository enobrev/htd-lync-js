...
export default class Protocol {
    ...
    command;
    data;
    constructor({ zone = Zone.Broadcast, command = Command.Common, data = Data.Off }) {
        this.header = Header;
    ...
    static reset_zone_audio(zone) {
        return new Protocol({ command: Command.Set_Audio_Default, zone });
    }
    static set_zone_name(zone, name) {
        if (name.length > 10) {
    ...
    static set_zone_source_name(zone, source, name) {
        if (name.length > 10) {
            name = name.substring(0, 10);
        }
        const data = [source];
